...
An IPv4 address is a numeric identifier that a device (or, on TV, hacker) uses to communicate on the internet, akin to a postal address in the
real world, typically formatted in dot-decimal notation as #.#.#.#. But each # should be a number between 0 and 255, inclusive.
    ...
    for item in list1:
        if len(item) > 1:
            if item[0] == "0":
                return False
    if (
        int(list1[0]) in range(0, 256)
        and int(list1[1]) in range(0, 256)
        and int(list1[2]) in range(0, 256)
        and int(list1[3]) in range(0, 256)
    ):
        return True
    return False
